...
(function ($) {
    ...
    $.modal = function (params) {
        ...
        if (params.buttons && params.buttons.length > 0) {
            for (var i = 0; i < params.buttons.length; i++) {
                buttonsHTML += '<span class="modal-button' + (params.buttons[i].bold ? ' modal-button-bold' : '') + (i == 1 ? params.mark && params.mark <= 0 ? ' modal-button-reject' : ' modal-button-pass' : '') + '">' + params.buttons[i].text + '</span>';
            }
        }
    ...
    $.promptReason = function (mark, callbackOk, callbackCancel) {
        // mark 0 拒绝 1 通过
        return $.modal({
            mark: mark || "0",
            extraClass: "modal-reason",
            afterText: '<div class="modal-inner-content"><textarea maxlength="20" id="text-wrapper" class="modal-text-area" placeholder="请输入' + (mark <= 0 ? "拒绝" : "通过") + '理由（非必填）"></textarea><div class="modal-text-count"><i class="count">20</i>/20</div></div>',
            buttons: [{
                text: "取消"
            }, {
                text: mark <= 0 ? "拒绝" : "通过"
            ...
            onClick: function onClick(modal, index) {
                if (index === 0 && callbackCancel) callbackCancel($(modal).find('textarea.modal-text-area').val());
                if (index === 1 && callbackOk) callbackOk($(modal).find('textarea.modal-text-area').val());
            }
        });
    };
    ...
    $.showIndicator = function () {
        ...
        showIndicatorTimer && clearTimeout(showIndicatorTimer);
        showIndicatorTimer = setTimeout(function () {
    ...
    $.hideIndicator = function () {
        ...
        clearTimeout(showIndicatorTimer) && (showIndicatorTimer = null);
    };
    // Action Sheet
    ...
    $(document).on('click', ' .modal-overlay, .popup-overlay, .close-popup, .open-popup, .close-picker', handleClicks).on('click', ' .modal-overlay-visible', function () {
        if ($('.modal.modal-in').length <= 0) {
            $.closeModal('.modal-overlay');
        }
    }).on('touchmove', ' .modal-overlay-visible', function (event) {
    ...
    function handleInput(e) {
        var el = $(e.target);
        var el_count = el.parent().find(".count");
        var val = el.val();

        // 取出回车字符
        var textareaVal = val.replace(/<(.+?)>/gi, "&lt;$1&gt;").replace(/\n/gi, "|");
        // 回车数量
        var entLen = textareaVal.split('|').length - 1;
        // 不包含回车的数量
        var strLen = textareaVal.split('|').join('').length;
        // console.log(textareaVal, entLen, strLen);

        el.attr('maxlength', 20 + entLen * 2);
        el_count.text(20 - strLen);

        if (val.indexOf("\\") >= 0) {
            var reg = new RegExp("\\\\", "g");
            el.val(val.replace(reg, ""));
            $.toast("您输入的内容中包含了特殊字符“" + "\\" + "”，系统已自动过滤！", 1500, "retoast");
        ...
        var emoji_regex = /[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF][\u200D|\uFE0F]|[\uD83C|\uD83D|\uD83E][\uDC00-\uDFFF]|[0-9|*|#]\uFE0F\u20E3|[0-9|#]\u20E3|[\u203C-\u3299]\uFE0F\u200D|[\u203C-\u3299]\uFE0F|[\u2122-\u2B55]|\u303D|[\A9|\AE]\u3030|\uA9|\uAE|\u3030/ig;
        if (emoji_regex.test(val)) {
            el.val(val.replace(emoji_regex, ""));
            $.toast("您输入的内容中包含了表情，系统已自动过滤！", 1500, "retoast");
        }
    };
    $(document).on('input propertychange', '#text-wrapper', handleInput);
